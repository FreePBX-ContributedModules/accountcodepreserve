...
function accountcodepreserve_hookGet_config($engine) {
	...
	*/
		$priority = 'report3';
		$ext->splice('macro-user-callerid', 's', $priority,new ext_execif('$["${CALLEE_ACCOUNCODE}" = ""]', 'Set', '__CALLEE_ACCOUNCODE=${DB(AMPUSER/${IF($["${ARG6}"="macro-exten-vm"]?${ARG2}:${ARG1})}/accountcode)}'));
	...
	*/
		...
		foreach ($routes as $route) {
			...
			foreach ($patterns as $pattern) {
				...
				$ext->splice($context, $extension, 2, new ext_execif('$[ "${CALLEE_ACCOUNCODE}" != "" ] ','Set','CHANNEL(accountcode)=${CALLEE_ACCOUNCODE}'));
			}
		}
